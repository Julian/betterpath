...
"""
Test cases covering L{bp.filepath}.
"""
...
from bp.win32 import isWindows, WindowsError, ERROR_DIRECTORY
from bp import filepath
...
class AbstractFilePathTestCase(BytesTestCase):
    ...
    def test_segmentsFromPositive(self):
        ...
        child = self.path.child(b"a").child(b"b").child(b"c")
        self.assertEqual(child.segmentsFrom(self.path), [b"a", b"b", b"c"])
    ...
    def test_validSubdir(self):
        """
        Verify that a valid subdirectory will show up as a directory, but not
        as a file, not as a symlink, and be listable.
        """
    ...
    def test_dictionaryKeys(self):
        ...
        # sanity check
        self.assertFalse(list(dictoid.keys())[0] is f1prime)
        dictoid[f2] = 5
    ...
    def test_dictionaryKeyWithString(self):
        """
        Verify that path instances are usable as dictionary keys which do not
        clash with their string counterparts.
        """
        f1 = self.path.child(b'file1')
        dictoid = {f1: 'hello'}
    ...
    def test_childrenNonexistentError(self):
        ...
        directories.
        """
        self.assertRaises(UnlistableError,
                          self.path.child(b'not real').children)

    def test_childrenNotDirectoryError(self):
        """
        Verify that listdir raises the appropriate exception for attempting to
        list a file rather than a directory.
        """
        self.assertRaises(UnlistableError, self.path.child(b'file1').children)

    def test_newTimesAreFloats(self):
        """
        Verify that all times returned from the various new time functions are
        floats (and hopefully therefore 'high precision').
        """
...
class FakeWindowsPath(filepath.FilePath):
    """
    A test version of FilePath which overrides listdir to raise
    L{WindowsError}.
    """
...
class ExplodingFile:
    ...
    track of whether it has been closed.

    @ivar closed: A C{bool} which is C{False} until C{close} is called, then
        it is C{True}.
    """
    closed = False
...
class TrackingFilePath(filepath.FilePath):
    """
    A subclass of L{filepath.FilePath} which maintains a list of all other
    paths created by clonePath.
    ...
    @ivar trackingList: A list of all paths created by this path via
        ...
        C{parent}, C{sibling}, C{child}, etc (and all paths subsequently
        created by those paths, etc).

    @type trackingList: C{list} of L{TrackingFilePath}

    @ivar openedFiles: A list of all file objects opened by this
        L{TrackingFilePath} or any other L{TrackingFilePath} in
        C{trackingList}.
    ...
    def clonePath(self, name):
        """
        Override L{filepath.FilePath.clonePath} to give the new path a
        reference to the same tracking list.
        """
...
class PermissionsTestCase(BytesTestCase):
    ...
    def test_permissionsFromStat(self):
        """
        L{Permissions}'s alternate constructor takes a valid permissions
        bitmask and parses it to produce the correct set of boolean
        permissions.
        """
        ...
        for u in range(0, 8):
            for g in range(0, 8):
                for o in range(0, 8):
                    ...
                    perm = filepath.Permissions.fromStat(chmodVal)
        ...
        perm = filepath.Permissions.fromStat(0o777)
    ...
    def test_permissionsEq(self):
        ...
        are equivalent
        """
        self.assertEqual(filepath.Permissions.fromStat(0o777),
                         filepath.Permissions.fromStat(0o777))
        self.assertNotUnequal(filepath.Permissions.fromStat(0o777),
                              filepath.Permissions.fromStat(0o777))
        self.assertNotEqual(filepath.Permissions.fromStat(0o777),
                            filepath.Permissions.fromStat(0o700))
        self.assertNotEqual(3, filepath.Permissions.fromStat(0o777))
    ...
    def test_permissionsShorthand(self):
        ...
        for u in range(0, 8):
            for g in range(0, 8):
                for o in range(0, 8):
                    perm = filepath.Permissions.fromStat(
                        int("0o%d%d%d" % (u, g, o), 8))
                    expected = ''.join(x.shorthand() for x in perm)
                    self.assertEqual(perm.shorthand(), expected)
        self.assertEqual(filepath.Permissions.fromStat(0o770).shorthand(),
                         "rwxrwx---")
...
class FilePathTestCase(AbstractFilePathTestCase):
    ...
    def test_realpathCyclicalSymlink(self):
        ...
        """
        self.symlink(self.path.child(b"link1").path,
    ...
    def test_copyToMissingDestFileClosing(self):
        ...
        # We won't get as far as writing to this file, but it's still useful
        # for tracking whether we closed it.
    ...
    def test_moveToSizeCache(self, hook=lambda: None):
        ...
        calls to L{FilePath.getsize} after the call to C{moveTo} will report
    ...
    def test_changed(self):
        """
        L{FilePath.changed} indicates that the L{FilePath} has changed, but
        does not re-read the status information from the filesystem until it
        is queried again via another method, such as C{getsize}.
        """
    ...
    def test_getPermissions_POSIX(self):
        ...
        for mode in (0o777, 0o700):
            ...
            self.assertEqual(self.path.child(b"sub1").getPermissions(),
                             filepath.Permissions.fromStat(mode))
        # sanity check
        self.path.child(b"sub1").chmod(0o764)
    ...
    def test_getPermissions_Windows(self):
        ...
        for mode in (0o777, 0o555):
            ...
            self.assertEqual(self.path.child(b"sub1").getPermissions(),
                             filepath.Permissions.fromStat(mode))
        # sanity check to make sure that user=group=other permissions
        self.path.child(b"sub1").chmod(0o511)
        self.assertEqual(self.path.child(b"sub1").getPermissions().shorthand(),
                         "r-xr-xr-x")
    ...
    if isWindows:
        ...
        msg = "Test will run only on Windows."
        test_statinfoBitsNotImplementedInWindows.skip = msg
        test_getPermissions_Windows.skip = msg
